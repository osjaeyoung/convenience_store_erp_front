...
- `action=complete`로 `completed` 전환 시 계약 정보 기준으로 근무현황이 자동 생성됩니다.
  ...
  - 하루에 여러 근무 구간이 있을 수 있으므로 서버는 `work_day_{n}_slots` 배열을 우선 처리합니다. 예: 같은 금요일에 `10:00~15:00`, `18:00~20:00` 두 번 근무할 수 있습니다.
  - `work_day_{n}_slots`가 있으면 기존 단일 필드(`work_day_{n}_start`, `work_day_{n}_end`, `work_day_{n}_break_*`)보다 우선합니다. 단일 필드는 하위호환 및 첫 번째 구간 표시용으로 유지됩니다.
  - 야간근무처럼 종료 시간이 시작 시간보다 이르거나 같은 경우(`23:00` ~ `06:00`, `23:00` ~ `23:00`)는 **익일 종료**로 해석합니다. 예: 금/토 `23:00~06:00`은 `work_day_5_start=23:00`, `work_day_5_end=06:00`, `work_day_6_start=23:00`, `work_day_6_end=06:00`처럼 저장하며, 서버는 금 23:00~토 06:00 / 토 23:00~일 06:00 슬롯으로 자동 배정합니다.
  - 자동 근무현황 생성 시 자정을 넘는 구간은 날짜별 슬롯으로 나누어 저장합니다. 예: 금 23:00~익일 06:00이면 금요일 `23:00~24:00` 구간과 토요일 `00:00~06:00` 구간에 해당하는 30분 슬롯을 생성합니다.
...
3. **체크박스 키 `work_day_*`**  
   ...
   - 요일별 근무시간은 `work_day_{n}_slots` 배열을 우선 사용합니다. `slots`가 없으면 기존 `work_day_{n}_start`, `work_day_{n}_end` 값을 fallback으로 사용합니다.
   - `end <= start`이면 다음날 종료로 해석합니다. 프론트는 야간근무를 같은 요일에 `23:00~06:00` 형태로 보냅니다.

#### 요일별 다중 근무구간 예시

```json
{
  "work_day_5_enabled": "1",
  "work_day_5_start": "10:00",
  "work_day_5_end": "15:00",
  "work_day_5_slots": [
    {
      "start": "10:00",
      "end": "15:00",
