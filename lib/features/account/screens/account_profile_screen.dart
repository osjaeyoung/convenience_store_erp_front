...
class _AccountProfileScreenState extends State<AccountProfileScreen> {
  ...
  Future<void> _saveNameIfChanged() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text(accountDioMessage(e))));
      }
    }
  }
  ...
  Future<void> _requestPhoneVerification() async {
    ...
    if (phoneNumber.isEmpty || phoneNumber.length < 10) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('올바른 휴대폰 번호를 입력해주세요.')));
      return;
    }
    ...
    if (phoneNumber == currentPhone) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('기존 번호와 동일합니다.')));
      return;
    }
    ...
    try {
      final result = await repo.checkPhoneNumberExists(
      ...
      if (result.exists) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('이미 가입된 전화번호입니다.')));
        return;
      }
  ...
  Widget _fieldBlock({required String caption, required Widget child}) {
    return Column(
